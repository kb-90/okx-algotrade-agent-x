...
class RiskConfig:
    ...
    position_ratios: list = None

    @classmethod
    def from_cfg(cls, cfg: dict):
        valid_keys = {
            'backtest_equity', 'leverage',
            'max_daily_loss', 'max_exposure', 'risk_per_trade', 'max_open_orders', 'position_ratios'
        }
...
class RiskManager:
    ...
    def get_position_size(self, df: pd.DataFrame, signal: int, close: float, position_level: int = 0, current_positions: list = None) -> float:
        ...
        # Fallback ATR calculation if not available
        if atr is None or atr <= 0:
            if isinstance(df, pd.DataFrame) and len(df) > 1:
                try:
                    # Calculate simple ATR as average of (high-low) over last 14 periods
                    recent_data = df.tail(14)
                    atr_values = (recent_data['high'] - recent_data['low']).values
                    atr = float(np.mean(atr_values))
                    logger.debug(f"RiskManager: Calculated fallback ATR: {atr}")
                except Exception:
                    atr = None
        ...
        # Apply scaling ratios for multiple positions first
        scaling_ratios = self.config.position_ratios or [1.0, 0.5]
    ...
    def _apply_dynamic_risk_adjustment(self, df: pd.DataFrame, base_units: float, close: float) -> float:
        ...
        # Apply dynamic adjustment but preserve scaling ratios by using a smaller multiplier
        # This ensures scaling ratios are not completely overridden
        if base_units > 0:
            # Use a more conservative multiplier that doesn't override scaling
            conservative_multiplier = min(equity_multiplier * 0.5, 2.0)  # Cap at 2x instead of 10x
            return base_units * conservative_multiplier
        else:
            return base_units
